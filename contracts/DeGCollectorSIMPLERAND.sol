...
/**
 * DeG 1 - DeGCollectorSIMPLERAND
 *
 * @author Tuckson
 * @title DeG 1 - DeGCollectorSIMPLERAND
 */
contract DeGCollectorSIMPLERAND is ERC721, Ownable, ReentrancyGuard {
    //
    ///////////////////////////////////
    ...
    uint8[] private _availableTokenTypes = [0, 1, 2, 3, 4, 5];
     ...
     *
     * Notes:
     *  - This fucntion gives the same probability for every type, so the
     *      most special ones get minted in the beginning and then there's only the others left
     */
    function getRandomTokenType() internal returns (uint8) {
        // Getting a random token type
        uint256 tokenTypeAvailableIndex = uint256(
            keccak256(
                abi.encodePacked(
        ...
        ) % _availableTokenTypes.length;

        // Getting the token type
        uint8 tokenTypeIndex = _availableTokenTypes[tokenTypeAvailableIndex];

        // Adjusts new supply for this type of token
        _tokenTypeSupply[tokenTypeIndex]++;

        // Updates available token types if needed
        if (_tokenTypeSupply[tokenTypeIndex] >= _tokenTypeMaxSupply[tokenTypeIndex]) {
            uint8[] memory newAvailableTokenTypes = new uint8[](_availableTokenTypes.length - 1);

            uint8 newArrayIdx = 0;
            for (uint8 i = 0; i < _availableTokenTypes.length; i++) {
                if (i != tokenTypeAvailableIndex) {
                    newAvailableTokenTypes[newArrayIdx] = _availableTokenTypes[i];
                    newArrayIdx++;
                }
            }

            _availableTokenTypes = newAvailableTokenTypes;
        }
